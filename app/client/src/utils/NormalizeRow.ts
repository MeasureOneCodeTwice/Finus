...
export function normalizeRow(raw: unknown): NormalizedRow {     //row is treated as an object with unknown values
  const record = raw as Record<string, unknown>;
 
  const clean = (value: unknown): string | null => { // helper to trim and clean string-like values
...
function normalizeDate(input: string | null): string | null {
  ...
  if (/^\d{2}-\d{2}-\d{4}$/.test(data)) {       // if MM-DD-YYYY or DD-MM-YYYY, convert to YYYY-MM-DD
...
function normalizeAmount(input: string | null): number | null {
  ...
  if (/^\(.+\)$/.test(amount)) {                    //if amount is in parentheses, treat as negative
