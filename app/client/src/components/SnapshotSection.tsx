...
function AccountCardsSection() {
  ...
  if (error) {
    return (
      <div className="text-center my-10">
        ...
        <button onClick={refresh} className="text-green-400 hover:underline">
          Try again
        </button>
      </div>
    );
  ...
  return (
    <section className="flex flex-row items-center justify-center gap-12 my-10 relative">
      {isLoading ? (
        ...
        <>
          <AccountCard
            ...
            amount={totalBalance}
            trend={
              parseFloat(totalBalance.replace(/[$,]/g, "")) >= 0
                ? "positive"
                : "negative"
            }
          />
      ...
      >
        <RefreshCw
          className={`w-4 h-4 text-green-400 ${isLoading ? "animate-spin" : ""}`}
        />
      </button>
    </section>
  );
