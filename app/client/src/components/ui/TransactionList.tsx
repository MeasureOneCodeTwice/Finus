...
export default function transactionList({accountId}:listProp) {
    ...
    const [seen, setSeen] = useState<boolean>(false)
        ...
        const addTransaction = (newTransaction:Transaction) => {
            setAccountTransactions(userTransactions => [...userTransactions,newTransaction])
        }
    
        //Removes any transaction from the list thats shares the same id
        const removeTransaction = (remove:Transaction) => {
            setAccountTransactions(userTransactions=> userTransactions.filter((transaction) => transaction.id !== remove.id))
        }
        ...
        return(
            ...
            <div>
                <div>
                {accountTransactions.map(transaction => (<TransactionCard transaction={transaction} setTransaction={addTransaction} removeTransaction={removeTransaction}/>))}
                </div>
            ...
            {seen ? (<TransactionPopup toggle={toggle} addTransaction={addTransaction} edit = {false} />):null}
            </>
        )
}