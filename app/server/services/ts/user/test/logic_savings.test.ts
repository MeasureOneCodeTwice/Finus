...
describe("getSavingAccountTransactionBy", () => {
   const mockFindTransactionsBy = transactionQueries.findTransactionsBy as vi.Mock;
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("should throw error if financialAccountId is missing", async () => {
    await expect(
      getSavingAccountTransactionBy(undefined)
    ).rejects.toThrow("Missing financial account id");
  });

  // Triggering db error 
  it("should throw if findTransactionsBy fails", async () => {
    mockFindTransactionsBy.mockRejectedValue(
      new Error("DB error")
    );

    await expect(
      getSavingAccountTransactionBy("1")
