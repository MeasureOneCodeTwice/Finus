...
import type { Goal, CreateGoalInput, UpdateGoalInput, GoalWithProgress } from "../types/Goals.ts";
...
async function calculateCurrentAmount(
  ...
  if (goal.type === 'spending_limit') {//TODO: append expenditures into current_amount in Goal type, also handle the case of savings
    //get the current period's spending, this can be either a week or a month
    const period = goal.period
    const now = new Date();
    let startOfWeek = now;
    let startOfMonth = now;

    if (period == "w"){
        startOfWeek = new Date(now.getFullYear(), now.getMonth(), now.getDate() - now.getDay());
        const spending = await transactionsQueries.getDateCategoryTransactionsQuery(
            pool,
        ...
        const total_spending = spending.reduce((total, transaction) => total + transaction.amount, 0);
...
export async function enrichGoalWithProgress(
  ...
  userId: number
    ...
    const progress_percentage = Math.min((current_amount / goal.target) * 100, 100);
...
export async function createUserGoal(
  ...
  if (goalCount >= MAX_GOALS_PER_PROFILE) {
    throw new Error(`Maximum ${MAX_GOALS_PER_PROFILE} goals allowed per profile`);
  }
  
  //validate input based on goal type
  validateGoalInput(goalData);
  
  return await goalsQueries.createGoal(pool, profileId, goalData);
}
...
export async function updateUserGoal(
  ...
  //validate updates
  validateGoalUpdates(existingGoal, updates);
  
  const updated = await goalsQueries.updateGoal(pool, goalId, profileId, updates);
  if (!updated) {
    throw new Error("Failed to update goal");
  }
  
  const updatedWithProgress = await enrichGoalWithProgress(pool, updated, profileId);
...
function validateGoalInput(input: CreateGoalInput): void {
  ...
  if (input.type === 'spending_limit' && !input.category) {
    throw new Error("Spending limit goals require a category");
  }
}

function validateGoalUpdates(goal: Goal, updates: UpdateGoalInput): void {
