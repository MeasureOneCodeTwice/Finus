...
export async function findDebtsBy(
  db: Pool,
  userId: string,
): Promise<DebtInfoResponse[]> {
  const query = `
...
export async function addDebt(
  db: Pool,
  newDebt: FinancialAccountRequest,
  userId: string,
): Promise<DebtInfoResponse> {
  ...
  try {
    ...
    const [profiles] = await connection.execute<RowDataPacket[]>(profileQuery, [
      userId,
    ]);
