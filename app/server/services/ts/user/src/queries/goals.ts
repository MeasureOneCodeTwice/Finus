  ...
  try {
    ...
    const [result] = await connection.query(
      `INSERT INTO finus.goal (name, type, category, target, period)
       VALUES (?, ?, ?, ?, ?)`,
      [goalData.name, goalData.type, goalData.category || 'unknown', goalData.target, goalData.period]
    );
...
export async function updateGoal(
  ...
  const allowedFields = ["name", "category", "target"];
...
export async function deleteGoal(
  ...
  try {
    ...
    await connection.query(
      `DELETE FROM finus.goal WHERE id = ?`,
      [goalId]
    );
