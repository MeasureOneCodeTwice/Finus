...
import { deleteUserGoal, createUserGoal, updateUserGoal, getUserGoals, enrichGoalWithProgress, getUserGoalById} from "./logic/goals.ts";
import type { UpdateGoalInput } from "./types/Goals.ts";
...
app.get("/table/transactions", async (req : express.Request, res : express.Response) => {
  try {
    const userId = authenticateJWT(req);
...
app.post("/goals", async (req: express.Request, res: express.Response) => {
  try {
    ...
    if (goalCount >= 5) {
      return res.status(400).json({ error: "Maximum 5 goals allowed per profile" });
    }
    
    const newGoal = await createUserGoal(pool, userId, { name, type, category, target, period});
